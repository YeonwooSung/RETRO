...
from pyretro.retrieval import EOS_ID
from pyretro.retro_utils import memmap
...
def knn_to_retrieved_chunks(
    ...
    *,
    ...
    is_last_document_chunk = np.any(
        knn_chunks == eos_id, axis=-1, keepdims=True)
...
class RETRODataset(Dataset):
    def __init__(
        ...
        *,
        ...
        self.get_seqs = partial(memmap, seq_memmap_path,
                                dtype=np.int32, shape=(num_sequences,))
    ...
    def __getitem__(self, ind):
        with self.get_chunks() as chunks_memmap, self.get_knns() as knns_memmap, self.get_seqs() as seqs_memmap:
            ...
            chunk_range = slice(begin_chunk_index,
                                (begin_chunk_index + self.seq_num_chunks))
